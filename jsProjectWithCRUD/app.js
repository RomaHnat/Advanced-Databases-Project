...
function loadDocumentForDeletion() {
    const searchId = document.getElementById('deleteSearchId').value.trim();
    if (!searchId) {
        showMessage('Please enter a document ID', 'error');
        return;
    }
    ...
    localDB.get(searchId).then(function (doc) {
        let html = '<div class="record-preview" style="border: 1px solid #red; margin: 10px 0; padding: 10px; background-color: #ffe6e6;">';
        html += '<h4>Record to be deleted:</h4>';
        html += '<p><strong>Document ID:</strong> ' + doc._id + '</p>';
        html += '<p><strong>Person ID:</strong> ' + doc["Person ID"] + '</p>';
        html += '<p><strong>Gender:</strong> ' + doc["Gender"] + '</p>';
        html += '<p><strong>Age:</strong> ' + doc["Age"] + '</p>';
        html += '<p><strong>Occupation:</strong> ' + doc["Occupation"] + '</p>';
        ...
        window.docToDelete = doc;

        showMessage('Document loaded. Please confirm deletion below.', 'success');
    }).catch(function (err) {
        if (err.name === 'not_found') {
            showMessage('Document not found with ID: ' + searchId, 'error');
        } else {
...
function deleteDocument() {
    ...
    localDB.get(deleteId).then(function (doc) {
        ...
        showMessage('Document deleted successfully!', 'success');
        document.getElementById('quickDeleteId').value = '';
    }).catch(function (err) {
        if (err.name === 'not_found') {
